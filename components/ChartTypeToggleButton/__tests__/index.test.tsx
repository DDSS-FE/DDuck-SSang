...
describe('ChartTypeToggleButton 컴포넌트는', () => {
  describe('Layout', () => {
    it('chartType이 candle일 때 line 아이콘을 보여준다.', () => {
      const setChartType: Dispatch<SetStateAction<ChartType>> = jest.fn();
      render(
        <ChartTypeToggleButton type="candle" setChartType={setChartType} />
      );
    ...
    it('chartType이 line일 때 candle 아이콘을 보여준다.', () => {
      const setChartType: Dispatch<SetStateAction<ChartType>> = jest.fn();
      render(<ChartTypeToggleButton type="line" setChartType={setChartType} />);
